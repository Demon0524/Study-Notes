#Linux  #Linux_Middleware #Nginx
[toc]
## 一、Nginx常用功能
- Nginx是一个高性能的HTTP和反向代理web服务器，同时提供了IMAP/POP3/SMTP服务。
- **Nginx是一款轻量级的web服务，占有内存少，并发能力强**

### 1. 正向代理&反向代理
![[../../../A-Accessories-Note附件/Nginx_正、反向代理.png]]
...
### 2. 负载均衡
- 通过分摊到多个操作单元上进行执行，从而共同完成工作任务
- Nginx提供了两种负载均衡策略：
	- 内置策略：轮询、加权轮询、Ip hash等方式
	- 扩展策略：任意扩展

> 轮询、加权轮询
![[../../../A-Accessories-Note附件/Nginx_轮询&加权轮询.jpg]]

> Ip hash
![[../../../A-Accessories-Note附件/Nginx_Ip hash算法.jpg]]


### 3. web缓存
Nginx可以对不同的文件做痛的缓存处理。可配合第三方的ngx_cache_purge，对定制的URL缓存内容可以的进行增删管理
...
1. 全局块：配置全局的nginx参数，如worker_processes、pid等。
2. events块：配置nginx的事件模型，如worker_connections、use等。
3. http块：配置http协议相关的参数，如access_log、error_log等。
4. http全局块：配置http协议相关的全局参数，如sendfile、tcp_nopush等。
5. server块：配置虚拟主机相关的参数，如listen、server_name等。
6. server全局块：配置虚拟主机相关的全局参数，如client_max_body_size、keepalive_timeout等。
7. location块：配置请求匹配的规则和处理方式，如proxy_pass、root等。
...
> nginx中的upstream模块用于定义一组后端服务器，以便nginx可以将请求分发到这些服务器上。upstream模块可以配置**负载均衡**算法、**健康检查**、**故障转移**等功能，以**提高系统的可用性和性能**。
...
# 这个配置定义了一个名为backend的upstream，其中包含两个服务器：192.168.1.1:8080和192.168.1.2:8080。
# nginx会将请求分发到这两个服务器上，以实现负载均衡。
```

- 负载均衡算法

```nginx
# upstream模块还支持多种负载均衡算法，例如轮询、IP哈希、最少连接数等。可以使用以下语法指定负载均衡算法：
upstream backend {
    ip_hash;
    server 192.168.1.1:8080;
    server 192.168.1.2:8080;
}
# 这个配置使用了ip_hash算法，它会根据客户端IP地址将请求分发到后端服务器上。这样可以确保同一个客户端的请求都被分发到同一个后端服务器上，以避免会话丢失等问题。
...
```nginx
# upstream模块还支持健康检查和故障转移功能。可以使用以下语法指定健康检查和故障转移配置：
upstream backend {
    server 192.168.1.1:8080;
    server 192.168.1.2:8080;
...
# 这个配置指定了每5秒进行一次健康检查，并保持32个长连接以提高性能。如果某个后端服务器出现故障，nginx会自动将请求转发到其他正常的服务器上，以确保系统的可用性。
```
